...
class Seq2GraphTransformer(nn.Module):
    def __init__(self,
        ...
        self.transformer = Transformer(d_model=emb_size,
                                       ...
                                       dropout=dropout)
        self.linear_parallel = nn.Linear(SRC_SEQ_LEN * emb_size + 2 * emb_size, emb_size)
    ...
    def forward(self,
        ...
        transformer_enc = self.transformer.encoder(src_emb, src_mask).flatten(start_dim=1)
        transformer_enc = torch.cat([transformer_enc,
                                     torch.zeros((1, 51680-2*self.emb_size-transformer_enc.shape[-1]), dtype=transformer_enc.dtype)],
                                    dim=-1)
        # print(transformer_enc.shape)
        ...
        for b in range(BATCH_SIZE):
            atom_pairs = torch.cat([atom_node_emb[b, lower_tri_indices[0]],
                                    ...
                                    transformer_enc[b].repeat(len(lower_tri_indices[0]), BATCH_SIZE)], dim=-1)
            outs = self.linear_parallel(atom_pairs)
        #     atom_pairs = torch.cat([atom_node_emb[b, lower_tri_indices[0]],
        #                             atom_node_emb[b, lower_tri_indices[1]]], dim=-1)
        #     # print(atom_pairs.shape)
    ...
    for batch, (batch_mol_atom_node_feats, batch_mol_atom_valency, batch_adjacency_matrices, batch_msp_in) in enumerate(dataloader):
        ...
        if mol_num_atoms > 4:
            ...
            if batch % 4000 == 0:
                loss, current = loss.item(), (batch + 1) * len(batch_adjacency_matrices)
                print(f"loss: {train_loss/current:>7f}  [{current:>5d}/{size:>5d}]")
    print(f"Accuracy: ", correct/count_atoms, "Test Loss:", train_loss/size)
